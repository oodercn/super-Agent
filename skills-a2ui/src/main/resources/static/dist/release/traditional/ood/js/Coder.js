ood.Class("ood.Coder", null,{
    Initialize:function(){
        ood.CSS.addStyleSheet(
            ...
            '.sh-con span{vertical-align:inherit;'+(ood.browser.ie?'zoom:0;':'')+'}'+
        ...
        this._profiles={
            'js':{
                ...
                keyword2: 'exists|isNull|isObj|isEmpty|isArr|isBool|isDate|isFun|isHash|isNumb|isStr|ood.arr|ood.bool|ood.cls|ood.date|ood.fun|ood.hash|ood.numb|ood.str|ood.id|_|'+
                ...
                special: /ood[\w\.]*|(\bon|before|after|set|get)[A-Z]\w*/
            },
            'css':{
                multicomment:this.$COM_REG.BLOCK_COMMENT,
    ...
    Static:{
        ...
        */
        formatText:function(code, type, reverse){
            ...
            if(type == 'html'){
                ...
                code = ood.replace(code, arr, true);
            }else{
                var arr=[],
                ...
                if(type=='css'){
                    ood.arr.insertAny(arr,[/\s+(\.)/.source, " $1"],2,true);
                    ood.arr.insertAny(arr,[/(\d*\.?\d+|\d+\.?\d*)(cm|em|ex|pt|px|%|\:)?/, " $0 "],-1,true);
                }
                // prepare space
                code=ood.replace(code,arr);
                var Brackets={};
        ...
        */
        formatHTML:function(code, type, arrActions, fold, id, height, callback){
            ...
            a=ood.copy(this._profiles[type]);
...
//                        arr.push([o, r?r:"<span class='"+s+"'>$0<\/span>"]);
            ...
            ood.arr.each(["string1", "string2", "number"],function(s){
                if(a[s]){
                    f(a[s],s);
                    delete a[s];
                }
            ...
            var aa=[];
            aa.push("<div id='"+_key+":"+id+":' class='sh' "+"style='"+(height?("overflow:hidden;height:"+height+(ood.isFinite(height)?"px":"")):"") +"'>");
            if(arrActions && arrActions[0]){
                aa.push("<div id='"+_key+"-"+'sh-cmd'+":"+id+":' class='sh-cmd'>");

                aa.push("<span class='sh-toggle' href='javascript:;' onclick='ood.Coder._toggle(this);'>"
                    +(fold ? " + " : " - ")
                    +"</span> <span class='cmd-ruler'></span>");

                ood.arr.each(arrActions,function(s){
                     aa.push("<a id='"+_key+"-"+s+":"+id+":' href='javascript:;' onclick='ood.Coder._action(this,\""+s+"\",arguments[0]);'>"+s+"</a>");
                });

                aa.push("<span>"+type+" source code viewer, powered by <a href='http://www.itjds.net' target='_blank' style='font-size:9px;color:#000157;'>EUSUI</a></span>");
                aa.push("</div>");
            }
            aa.push("<pre style='display:none'>");
            aa.push(_str.replace(/<([\w\/])/g,"&lt;$1"));
            aa.push("</pre>");
            aa.push("<div id='"+_key+"-"+'sh-con'+":"+id+":'class='sh-con' "+"style='"+(height?("overflow:auto;height:"+height+(ood.isFinite(height)?"px":"")+";"):"") +(fold?"display:none;":"")+"'><ol id='"+_key+"-"+'ol'+":"+id+":' start='1' class='"+type+"'><li>");
            aa.push(alist.join('&nbsp;</li><li>'));
            aa.push("</li></ol></div>");
            aa.push("</div>");
            _encode=_decode=null;

            ood.asyRun(function(){ood.Coder._remedy(id, callback)});

            return aa.join('');
        },
        formatAll:function(code, type, arrActions, fold, id, height){
            var arr = ood.toArr(arguments);
        ...
        applyById:function(id, formatAll){
            var i = 0, self = this, fun = function(){
                if(ood.Dom.byId(id)){
                    ...
                    o = ood.str.toDom(ood.Coder[formatAll ? "formatAll" : "formatHTML"](_t.text(), cls[0], cls[1]&& cls[1].split("-"), cls[2], id + ":" + i));
            ...
            ood.Thread.repeat(fun);
        },
        _remedy:function(id, callback){
            var _t=this.$key+":"+id+":";
            if(!(_t=ood(_t)).isEmpty()){
                var p=_t.parent(), w=p.scrollWidth(), w2 = p.width();
                p.css('positoin','relative');
                if(w>w2)_t.width(w);
                ood.tryF(callback,[_t, p])
            }
        },
        $action:{
            run:function(o,e){
                var code=ood(o).parent().next().text(),
                    fun=new Function([], code);
                fun.call(ood(o).parent(2),e);
            },
            plain:function(o){
                var code = ood(o).parent().next().text();
