ood.Class("ood.UI.MDialog", "ood.UI.Widget", {
    Instance: {
        // 添加 iniProp 对象来存储默认值
        iniProp: {
            "dock": "height",
            "showEffects": "Flip H",
            "hideEffects": "Flip H",
            "width": "26.515151515151512em",
            "height": "45.45454545454545em",
            "caption": "弹出框",
            "minWidth": "350",
            "status": "right",
            "panelBgClr": "#FFFFFF"
        ...
        show: function (parent, modal, left, top, callback, ignoreEffects) {
            ...
            return this.each(function (profile) {
                ...
                if (p.status == 'right') {
                    top = 0;
                    left = Math.max(0, (pr.width() / scale - profile.$px(p.width)) + pr.scrollLeft() / scale)
                }
                var f1 = function () {
                    parent.append(ins);
                    var box = profile.box,
                ...
                profile.inShowing = 1;
                if (t = p.fromRegion)
                    profile.$inThread = ood.Dom.animate({border: 'solid 1px var(--border-dark)', background: 'var(--bg-secondary)', opacity: .1}, {
                        left: [t.left, left],
                        top: [t.top, top],
        ...
        hide: function (ignoreEffects) {
            this.each(function (profile) {
                ...
                var fun = function () {
                    ...
                    //max has dock prop
                    if (pro.status == 'max' || pro.status == 'min' || status == 'left' || status == 'right') {
                        var os = pro.status;
    ...
    Static: {
        ...
        DataModel: {
            ...
            status: {
                ini: 'normal',
                listbox: ['normal', 'min', 'max', 'left', 'right'],
                action: function (v, o) {
        ...
        _prepareData: function (profile) {
            ...
            if (status == 'min' || status == 'max' || status == 'left' || status == 'right')
                profile.$noR = 1;
        ...
        _modal: function (profile) {
            ...
            if (!p.isEmpty()) {
                if (!profile.$inModal) {
                    ...
                    var h = profile.$focusHash={}, b=ood('body').query('*',function(o){return o.tabIndex>0}).get();
                    for(var i=0,o;o=b[i++];){
                    ...
                    ood.Event.pushTabOutTrigger(profile.renderId, function(src,tabindex){
                        tabindex = parseInt(tabindex||1 +"",10);
                        var children = ood.use(src).get(0).getElementsByTagName('*'),t,n;
                        for(var i=0,l=children.length,o;o=children[i];i++){
                            if(o.nodeType==1){
        ...
        _unModal: function (profile) {
            if (profile.$inModal) {
                ...
                ood.breakO(profile.$focusHash,2);
                ood.Event.popTabOutTrigger();
                */
        ...
        _refreshRegion: function (profile) {
            ...
            return ood.merge(prop, nr, 'all');
            // return ood.merge(prop, nr, function(o,i){return prop[i]!='auto'});
        },
        ...
        alert: function (title, content, onClose, btnCap, left, top, parent, subId, noCache) {
            var me = arguments.callee, dialog, path;

            content = content ? content.replace('<br\/>', '/n') : title;
            if (noCache || !(dialog = me.dialog) || !dialog.get(0) || (!dialog.get(0).renderId)) {
                dialog = new ood.UI.MDialog({
                    overflow: 'hidden',
                    displayBar: false,
                    shadow: false,
                    width: "20em",
                ...
                var svgPaper = new ood.UI.SVGPaper({
                    dock: 'fill',
                    activeAnim: "zoomAlert"
                })
                ...
                var cmd = dialog.$cmd = new ood.UI.Div({
                        ...
                        borderType: "none",
                        zIndex: 10
                    }, null, null, null, {KEY: "text-align:center;padding-top:.25em;border-top:dotted 2px var(--border-muted);background-color: transparent;border-width:0em"}),
                    btn = dialog.$btn = new ood.UI.Button({
                            position: 'relative',
                            width: "18em",
                            height: "2em",
                            tabindex: 1
                ...
                cmd.append(btn);
                dialog.append(cmd).render();
                svgPaper.append(path);
                dialog.append(svgPaper).render();
        ...
        confirm: function (title, caption, onYes, onNo, btnCapYes, btnCapNo, left, top, parent, subId, noCache) {
            ...
            if (noCache || !(dialog = me.dialog) || !dialog.get(0) || (!dialog.get(0).renderId)) {
                ...
                var cmd = dialog.$cmd = new ood.UI.Div({
                        ...
                        borderType: "none",
                        zIndex: 10
                    }, null, null, null, {KEY: "text-align:center;padding-top:.25em;border-top:dotted 2px #666666;background-color: #ffffff00;border-width:0em"}),
                    btn = dialog.$btn1 = new ood.UI.SButton({
                            tabindex: 1,
                            position: 'relative',
                            imageClass: "ood-icon-right",
                            width: "8em",
                            height: "2em"
                ...
                var path = new ood.svg.pathComb({
                    ...
                    attr: {
                        ...
                        "KEY": {
                            "fill": "204-#6BBED2:0-#23538A:50-#82BBD1:100",
                            "stroke": "#000000",
                            "path": "M,10,10L,49.999899999999975,10L,49.999899999999975,10L,110.00001249999997,10L,250,10L,250,87.91713545950735L,250,87.91713545950735L,250,120.8634851437852L,250,143.57222430199647L,110.000011,143.57222430199647L,10.388461780000018,209.0416495374849L,50.00000619999998,143.57222430199647L,10,143.57222430199647L,10,121.31018811782678L,10,87.91713545950735L,10,87.91713545950735Z",
                            "x": 10,
                            "y": 10
                        },
                        "TEXT": {
