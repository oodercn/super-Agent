/**
 * 通知公告组件 - APICaller集成版本
 * 按照APICaller在iniComponents中静态初始化的模式实现
 */
ood.Class("ood.Mobile.OA.NoticeDesignerAPICaller", 'ood.Module',{
    Instance:{
        initialize : function(){
            // 组件初始化
        ...
        functions:{
            ...
            markAsRead: function(index) {
                ...
                if (index >= 0 && index < notices.length) {
                    var oldStatus = notices[index].read ? 'read' : 'unread';
                    ...
                    if (this.properties.actions && this.properties.actions.onStatusChange) {
                        ood.pseudocode._callFunctions(this.properties.actions.onStatusChange, [{
                            index: index,
            ...
            removeNotice: function(index) {
                var notices = this.getNotices();
                if (index >= 0 && index < notices.length) {
                    var removedNotice = notices.splice(index, 1)[0];
                    this.setNotices(notices);
                    
                    // 执行关联动作 - 使用标准的 OOD 框架动作定义方式
                    if (this.properties.actions && this.properties.actions.onDelete) {
                        ood.pseudocode._callFunctions(this.properties.actions.onDelete, [{
                            index: index,
            ...
            // 绑定数据源 - 使用标准的 OOD 框架动作定义方式
            bindDataSource: function(dataSource) {
                // 通过设计器中静态初始化的APICaller实例处理数据源交互
                if (dataSource && dataSource.url && this.noticeApiCaller) {
                    // 配置APICaller实例
                    this.noticeApiCaller
                        .setQueryURL(dataSource.url)
                        .setQueryMethod(dataSource.method || "GET");
                    
                    if (dataSource.params) {
                        this.noticeApiCaller.setQueryData(dataSource.params);
                    }
                    
                    // 发起请求
                    this.noticeApiCaller.invoke();
            ...
            executeAction: function(action, params) {
                if (action && typeof action === 'function') {
                    action(params);
                } else if (action && typeof action === 'object' && action.actions) {
                    // 如果是动作对象配置，使用 OOD 框架的标准方式处理
                    ood.pseudocode._callFunctions(action, [params], this.getModule());
            ...
            markAllAsRead: function() {
                ...
                // 执行关联动作 - 使用标准的 OOD 框架动作定义方式
                if (this.properties.actions && this.properties.actions.onBatchRead) {
                    ood.pseudocode._callFunctions(this.properties.actions.onBatchRead, [{changedNotices: changedNotices}], this.getModule());
                }
            }
        },
        iniComponents : function(){
            ...
            // 添加点击事件到具体的组件中，参考 ColInfo.js 的事件处理模式
            noticeItem1.onClick([
                {
                    "desc": "设置选中通知",
                    "type": "function",
    ...
    Static:{
        Templates: {
            tagName: 'div',
            className: 'ood-mobile-notice-apicaller',
            style: '{_style}',
