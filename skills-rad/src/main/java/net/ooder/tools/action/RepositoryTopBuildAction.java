...
@Controller
@RequestMapping(value = {"/action/table/build/"})
@MenuBarMenu(menuType = CustomMenuType.TOP, caption = "仓储编译", index = 3, imageClass = "ri-database-2-line")
@Aggregation(type = AggregationType.MENU, userSpace = UserSpace.SYS)
public class RepositoryTopBuildAction {
    public RepositoryTopBuildAction() {
    ...
    @RequestMapping(method = RequestMethod.POST, value = "rebuildTable")
    @CustomAnnotation(index = 0, caption = "创建库表", imageClass = "ri-add-line")
    ...
    @ResponseBody
    public ResultModel<Boolean> rebuildTable(String currentClassName, String projectName) {
...
//                DSMFactory.getInstance().getAggregationManager().genAggViewJava(viewRoot, viewBean, euModule.getClassName(), getCurrChromeDriver());
    ...
    @RequestMapping(value = {"syncEntity"}, method = {RequestMethod.POST})
    @APIEventAnnotation(customRequestData = {RequestPathEnum.SPA_PROJECTNAME, RequestPathEnum.SPA_CLASSNAME},
            beforeInvoke = CustomBeforInvoke.BUSY,
            afterInvoke = CustomCallBack.FREE,
            afterInvokeAction = @CustomAction(name = "reloadPage", method = "call", type = ActionTypeEnum.other, target = "callback", args = {"{SPA.reloadPage()}"}),
    ...
    @CustomAnnotation(index = 1, caption = "同步实体", imageClass = "ri-refresh-line")
    @ResponseBody
    public ResultModel<Boolean> syncEntity(String projectName, String currentClassName) {
        ResultModel resultModel = new ResultModel();
    ...
    @CustomAnnotation(index = 3, caption = "销毁重建", imageClass = "ri-refresh-line")
    @ResponseBody
