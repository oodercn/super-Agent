...
@MenuBarMenu(menuType = CustomMenuType.TOP, caption = "模块编译", index = 1, imageClass = "ri-settings-3-line")
...
public class MobuleBuildAction {
    ...
    @CustomAnnotation(index = 0, caption = "创建模型", imageClass = "ri-add-line")
...
//                DSMFactory.getInstance().getAggregationManager().genAggViewJava(viewRoot, viewBean, euModule.getClassName(), getCurrChromeDriver());
    ...
    @CustomAnnotation(index = 3, caption = "重置", imageClass = "ri-refresh-line")
    @ResponseBody
