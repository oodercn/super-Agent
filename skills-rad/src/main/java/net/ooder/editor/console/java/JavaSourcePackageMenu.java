...
import net.ooder.dsm.editor.agg.menu.JavaAggPackageMenu;
...
/**
 * Java源代码包菜单控制器类
 * 负责处理Java源代码包的上下文菜单操作
 */
@Controller
@RequestMapping(value = {"/rad/file/java/context/"})
@MenuBarMenu(menuType = CustomMenuType.CONTEXTMENU, caption = "菜单")
@Aggregation(type = AggregationType.MENU, rootClass = JavaAggPackageMenu.class, userSpace = UserSpace.SYS)
public class JavaSourcePackageMenu {
    ...
    @APIEventAnnotation(beforeInvoke = CustomBeforInvoke.CONFIRM, customRequestData = RequestPathEnum.TREEVIEW, callback = CustomCallBack.TREERELOADNODE)
    public @ResponseBody
