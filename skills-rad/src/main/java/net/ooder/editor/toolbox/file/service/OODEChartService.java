...
@RequestMapping("/rad/file/echart/")
@Controller
public class OODEChartService {

    @APIEventAnnotation(bindTreeEvent = CustomTreeEvent.RELOADCHILD, autoRun = true, bindTabsEvent = CustomTabsEvent.TABEDITOR)
    @RequestMapping("OODECharts")
    @TreeViewAnnotation
    @ModuleAnnotation(cache = false)
    @ResponseBody
    public TreeListResultModel<List<OODFileTree>> getOODECharts(String projectName) {
        return null;
    }
