...
@Controller
@RequestMapping(value = "/rad/file/module/context/")
@Aggregation(type = AggregationType.MENU, rootClass = ModuleContextMenu.class)
@MenuBarMenu(menuType = CustomMenuType.CONTEXTMENU, caption = "页面右键菜单")
public class ModuleContextMenu extends FileContextMenu {
    ...
    public @ResponseBody
    ResultModel<Boolean> copy(String path, String packageName) {
        ResultModel<Boolean> result = new ResultModel<Boolean>();
    ...
    @RequestMapping(value = {"buildRoute"}, method = {RequestMethod.POST})
    ...
    @CustomAnnotation(index = 0, caption = "编译视图", imageClass = "ri-bug-line")
    @ResponseBody
    public ResultModel<OODFileTree> buildRoute(String domainId, String projectName, String currentClassName) {
        ...
        try {
            ...
            MethodConfig methodAPIBean = euModule.getComponent().getMethodAPIBean();
            if (euModule != null) {
                List<ModuleComponent> moduleComponents = euModule.getComponent().findComponents(ComponentType.MODULE, null);
            ...
            resultModel.setData(new OODFileTree(euModule,projectName,domainInst.getDomainId()));
        } catch (Exception e) {
            e.printStackTrace();
    ...
    @CustomAnnotation(index = 1, caption = "更新领域模型", imageClass = "ri-hammer-line")
    ...
    @CustomAnnotation(index = 17, caption = "清空视图重建", imageClass = "ri-settings-3-line")
    @ResponseBody
    public ResultModel<OODFileTree>  reBuildEntity(String projectName, String currentClassName) {
        ResultModel resultModel = new ResultModel();
        ChromeProxy chrome = getCurrChromeDriver();
        try {
            if (projectName == null) {
            ...
            if (moduleComponent != null && moduleComponent.getMethodAPIBean() != null) {
                MethodConfig methodConfig = moduleComponent.getMethodAPIBean();
                List<ModuleComponent> moduleComponents = moduleComponent.findComponents(ComponentType.MODULE, null);
