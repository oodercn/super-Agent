...
在数字化转型的浪潮中，Agent技术正逐步成为企业业务系统的标配。Ooder作为新一代的Agent框架，为企业提供了安全、灵活、高效的业务集成和自动化解决方案。企业部署Ooder的必要性主要体现在以下几个方面：

1. **业务系统集成需求**：现代企业通常拥有多个独立的业务系统，Ooder提供了统一的集成框架，实现系统间的无缝连接和数据共享。

2. **SKILL开发便捷高效**：Ooder的SKILL开发框架简化了开发流程，支持快速构建和部署各类业务功能模块，大幅降低了开发成本和时间。

3. **开源SKILL生态繁荣**：随着开源SKILL的爆炸性增长，企业可以直接利用丰富的开源资源，加速业务创新和数字化转型。

4. **安全合规要求**：Ooder提供了完善的安全机制，包括身份认证、权限控制、数据加密等，帮助企业满足严格的安全合规要求。

5. **系统扩展性和灵活性**：Ooder的模块化设计和松耦合架构支持企业根据业务需求灵活扩展系统功能，快速响应市场变化。

6. **成本效益优化**：通过Ooder的自动化和集成能力，企业可以减少人工干预，提高运营效率，降低整体IT成本。

如何安全、规范地搭建和管理Agent系统，成为企业数字化转型的关键。本术语表旨在为企业提供统一的Ooder术语体系，确保系统开发、部署和使用过程中的一致性和规范性。

### 1.2 文档目的

本文档定义了Ooder系统的核心术语，旨在确保所有用户、开发者和系统管理员对Ooder的概念、组件和功能有统一的理解，促进系统开发、部署和使用过程中的有效沟通。

### 1.3 术语范围

本术语表涵盖Ooder系统的核心概念、扩展组件、设计原则和文档规范等相关术语，包含部分IoT领域的专业术语以支持边缘计算场景。

### 1.4 术语分类

Ooder术语分为五大类：
- **核心概念术语**：系统的基本构建块和核心功能
- **IoT相关术语**：支持IoT边缘计算场景的专业术语
...
| **MCPAgent** | Master Control Point Agent | MCP | 服务流入口，管理和协调其他Agent，用户可以拥有多个；在网关场景中作为网络协调器角色，负责网络管理和设备接入 | 作为用户访问入口，管理EndAgent和Skillflow，支持个人私有、公司所有和项目所有三种类型 |
| **RouteAgent** | Route Agent | Route | 负责桥接和转发，连接MCPAgent和EndAgent；在网关场景中作为中继/路由设备，负责信号中继和数据转发 | 在MCPAgent和EndAgent之间建立通信桥梁，支持复杂网络拓扑 |
...
| **Skillflow** | Skillflow | - | 多个Agent协作完成的服务流程，定义了执行步骤和逻辑，通常是跨MCP服务的；分为预定义和自由流转两种模式，主要用于企业和个人场景；网关场景中SKILL内部自行定义流程，需要与外部交互时通过MCPAgent完成与Skillflow的交互 | 编排多个EndAgent和SKILL，实现复杂业务流程 |
...
| **AIServer认证中心** | AIServer Authentication Center | - | 负责SKILL的认证、Cat功能注册和密钥管理；在网关场景中负责设备认证、安全管理和固件升级 | 提供身份认证、密钥管理和安全控制服务 |
| **AI Bridge** | AI Bridge | - | MCP与AIServer之间的连接桥梁，负责命令的发起和转发，频道数据的记录和操作 | 连接MCPAgent和AIServer，实现命令转发和数据同步 |
| **北上协议** | Northbound Protocol | - | 以mcpAgent为中心，向云服务（包括公云、私有云、混合云）通信的协议统称，也称为AI Bridge协议。在混合云场景下，可能会涉及公共运营商角色 | 定义mcpAgent与云服务之间的通信标准，支持多种云部署模式 |
| **南下协议** | Southbound Protocol | - | mcpAgent向下与其他组件通信的协议统称，包括mcpAgent与routeAgent、endAgent以及它们的程序载体skills之间的通信 | 定义mcpAgent与下层组件之间的通信标准，支持分布式部署 |
| **Agent协议** | Agent Protocol | - | 南下协议的具体实现，用于mcpAgent、routeAgent、endAgent以及它们的程序载体skills之间的通信 | 实现Agent之间的标准化通信，支持消息路由、状态同步和命令执行 |
| **AI Bridge协议** | AI Bridge Protocol | - | 北上协议的具体实现，用于mcpAgent与云服务之间的通信 | 实现与云服务的标准化通信，支持命令转发、数据同步和状态报告 |
| **Agent SDK** | Agent SDK | - | 南下协议的开发工具包封装，提供标准化的API和工具，简化Agent协议的实现 | 开发者使用Agent SDK快速集成Agent协议，减少开发工作量 |
| **AI Bridge SDK** | AI Bridge SDK | - | 北上协议的开发工具包封装，提供标准化的API和工具，简化AI Bridge协议的实现 | 开发者使用AI Bridge SDK快速集成AI Bridge协议，支持多种云服务 |
...
| **设备** | Device | - | 物理或虚拟的设备实体，具有唯一标识，可被管理和控制，部署在Zone上 | 代表物理或虚拟设备，如传感器、执行器、服务器等 |
| **场景** | Scene | - | 基于特定业务需求的Agent协作模式定义，包含协作规则、流程和数据交互方式 | 定义特定业务场景下的Agent协作模式 |
| **群组** | Group | - | 为实现特定Scene而形成的Agent集合，通过频道进行通信和协作 | 组织Agent实现特定场景的协作 |
| **Cap** | Cap | - | SKILL的频道上下文，代表SKILL的一个能力域或功能频道，包含disk、group和links等数据 | 管理SKILL的频道数据和上下文 |
| **频道** | Channel | - | 基于Cap形成的逻辑通信和数据管理通道，用于Group内Agent的协作 | 实现Group内Agent的通信和数据共享 |
| **VFS** | Virtual File System | VFS | 虚拟文件系统，用于Cap数据的本地-云端同步和管理 | 提供统一的文件系统接口，实现数据的本地-云端同步 |

### 2.2 IoT相关术语

| 术语 | 英文全称 | 简称 | 描述 | 场景使用 |
|------|----------|------|------|----------|
| **Agent唯一标识** | Agent Unique Identifier | AgentID | Agent的全局唯一标识，用于系统内识别和通信 | IoT设备的唯一标识，用于系统管理和通信 |
| **Agent MAC地址** | Agent MAC Address | AgentMAC | Agent设备的物理地址，用于网络通信 | 物联网设备的物理地址，用于网络通信和识别 |
| **Agent序列号** | Agent Serial Number | AgentSN | Agent设备的生产序列号，用于设备标识和追溯 | 设备的生产序列号，用于设备追溯和管理 |
| **链路质量指标** | Link Quality Indicator | LQI | 衡量Agent之间通信链路质量的指标，取值范围0-255，值越高表示链路质量越好 | 评估IoT设备之间的通信质量 |
| **父节点标识** | Parent Node Identifier | ParentNodeID | 在树形网络结构中，Agent的父节点唯一标识 | 在树形网络拓扑中标识设备的父节点 |
| **网络地址** | Network Address | NWKAddr | Agent在网络中的逻辑地址，用于网络通信中的快速寻址 | IoT设备在网络中的逻辑地址，用于快速寻址 |
| **扩展网络标识符** | Extended Network Identifier | ExtPANID | 扩展的网络标识符，用于区分不同的网络 | 区分不同的IoT网络，确保设备正确加入网络 |
| **网络密钥** | Network Key | NetKey | 用于网络通信加密的密钥 | 加密IoT设备之间的网络通信，确保数据安全 |
...
| **Endpoints** | Endpoints | - | 网络通信的端点列表，由协议、主机、端口和路径组成，用于访问SKILL接口 | 提供多个网络访问地址，支持负载均衡和高可用 |
...
| **A2A访问** | Agent to Agent Access | - | 两个SKILL之间通过Cap通讯端点直接交互的访问方式 | 实现SKILL间的安全通信和功能调用 |
...
| **复用Endpoints** | Endpoints Reuse | - | 同一SKILL的所有Capability共享同一个网络端点列表，通过Capability标识符实现内部路由 | 减少网络配置复杂度，便于SKILL的统一管理和访问 |
...
1. **大小写规则**：
   - 核心概念术语首字母大写（如SKILL、MCPAgent、Capability等）
   - 简称使用大写（如MCP、CAT等）
   - 普通术语使用小写（如user、endpoints等）

2. **术语替换规则**：
    - 替换所有使用"Cat"或"cat"表示能力分类的地方，统一使用"Capability"或"cap"
    - 保持"CAT"作为"Capability And Type"的缩写，用于能力分类
    - 避免使用"能力"的其他同义词，如"功能"、"接口"等，统一使用"Capability"
    - 注意"Cat功能"术语已废弃，所有文档和代码中应使用"Capability"替代
    - 注意"Gateway"术语已废弃，由MCPAgent替代
    - 注意"Sensor"术语已废弃，由Agent替代
    - 替换所有使用"Place"的地方，统一使用"Space"
    - 替换所有使用"Area"的地方，统一使用"Zone"
    - 替换所有使用"DeviceEndPoint"的地方，统一使用"Device EndPoint"
    - 替换所有使用"SuperAgent"的地方，统一使用"Ooder"
    - 替换IoT相关术语：将"ieee"改为"agentSn"，"parentNodeieee"改为"parentNodeId"，"lqi"改为"lqi"（保留缩写，但更新描述）
    - 替换所有使用"endpoint"的地方，统一使用"endpoints"（复数形式）

3. **概念层级规则**：
   - 能力定义（逻辑层）与网络实现（物理层）分离
   - SKILL是物理程序，EndAgent是功能点，Capability是具体能力
   - Endpoints是网络通信的实现细节
   - Space是资源容器，Zone是原子部署单元

### 3.2 1:n Capability关系使用

- 一个SKILL可以提供多个Capability
- Capability由SKILL开发者静态定义
- 支持细粒度的权限控制和能力发现
- 通过CAT实现Capability的分类和组织

### 3.3 Endpoints使用

- 同一SKILL的所有Capability共享同一个网络端点列表
- 通过消息头中的Capability标识符实现内部路由
- 减少网络配置复杂度，便于统一管理
- Endpoints格式：`{protocol}://{host}:{port}/{api_version}/{cat}/{skill_id}/{interface_path}`

### 3.4 Space和Zone使用

- Space是资源的容器单元，用于组织和管理Zone，允许嵌套
- Zone是资源的原子部署单元，直接承载SKILL实例，不可嵌套
- 设备和SKILL部署在Zone上，Zone属于某个Space
- 支持多租户和权限隔离
...
Ooder系统适用于多种业务场景，以下是一个典型的文件分析服务案例：

#### 4.1.1 场景概述

企业需要实现一个文件分析服务，包括文件上传、存储和内容分析三个步骤。使用Ooder系统实现该服务可以充分利用其模块化、可扩展的架构优势。
...
| SuperAgent | Ooder | 系统名称 |
| ieee | agentSn | IoT设备标识符 |
| parentNodeieee | parentNodeId | 父节点标识符 |
| macno | agentMac | 设备MAC地址 |
| factory | agentVendor | 设备厂商 |
| serialno | agentSn | 设备序列号 |
| deviceId | agentId | 设备ID |
| deviceName | agentName | 设备名称 |
| endpoint | endpoints | 网络通信端点（从单数改为复数） |

### 4.3 完整流程示例

以下是一个完整的SKILL开发-发布-使用-管控流程示例，展示了各角色如何在不同阶段使用Ooder术语：
...
- **系统管理员**：协助部署和配置SKILL
- **核心术语**：SKILL注册中心、AIServer认证中心、SKILL部署环境、Endpoints
- **关键活动**：
  - SKILL打包和版本标记
  - 在SKILL注册中心注册SKILL
  - 通过AIServer认证中心完成认证
...
#### 4.4.1 A2A（Agent to Agent）密钥交换流程
...
#### 4.4.2 A2C（Agent to Cloud）密钥认证和交换流程
...
4. **授权检查**：验证第三方MCPAgent是否具有接入Ooder系统的权限
...
- Ooder AI Bridge协议设计文档
- Ooder Agent通讯机制设计文档
- Ooder Scene与Group体系设计文档
- Ooder Skill Cap频道上下文设计文档
- 物联网安全标准与最佳实践
- 分布式系统设计原则
- 公钥基础设施（PKI）技术规范