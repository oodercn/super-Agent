...
Agent 是 Ooder 智能体系统中的核心组件，负责执行任务、管理资源和与其他组件通信。Agent 协议是**南向协议**的具体实现，用于 mcpAgent、routeAgent、endAgent 以及它们的程序载体 skills 之间的通信。
### 1.1 协议体系定位

- **北向协议**：以 mcpAgent 为中心，向云服务（包括公云、私有云、混合云）通信的协议统称，也称为 AI Bridge 协议
- **南向协议**：mcpAgent 向下与其他组件通信的协议统称
- **Agent 协议**：南向协议的具体实现，本手册详细定义此协议
- **AI Bridge 协议**：北向协议的具体实现，用于与云服务通信
...
- **层次化架构**：形成从中心到边缘的层次结构
- **通信标准化**：使用统一的 Agent 协议（南向）和 AI Bridge 协议（北向）
- **高可用性**：支持故障转移和负载均衡
