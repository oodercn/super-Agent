package net.ooder.sdk.api.event.impl;

import net.ooder.sdk.api.event.*;
...
public class EventBusImpl implements EventBus {
            ...
            public R get() {
                ...
                for (EventHandler<?> handler : eventHandlers) {
                    try {
                        ...
                        if (handler instanceof ResultEventHandler) {
                            @SuppressWarnings("unchecked")
                            ResultEventHandler<T, R> resultHandler = (ResultEventHandler<T, R>) handler;
                            result = resultHandler.handleWithResult(event);
                        }
                    } catch (Exception e) {
                        log.error("Error handling event: {}", eventType.getSimpleName(), e);
                    }
    ...
    public interface ResultEventHandler<T extends Event, R> extends EventHandler<T> {
        R handleWithResult(T event);
    }
