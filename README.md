...
最新更新 (v0.6.6 Maven Central)：
- **依赖管理优化**：从GitHub仓库迁移到Maven Central，使用net.ooder 2.0版本依赖
...
**注意**：本工程是一个一体化的调试工程，部署和调试过程相对复杂，适合有一定经验的开发者使用。

### 新手入门建议

对于新手开发者，建议首先从 **[ooder-Nexus](https://github.com/oodercn/ooder-Nexus)** 项目入手：
- ooder-Nexus 提供了更简化的部署流程和完整的基础功能
- 包含详细的入门文档和示例代码
- 适合快速理解 ooder SuperAgent 的核心概念和基本使用方法

### 主开发环境

当您完成基础部署并熟悉了 ooder SuperAgent 的基本功能后，可以将本工程作为主开发环境：
- 本工程提供了完整的 SDK 开发和调试环境
- 支持更复杂的场景测试和功能扩展
- 包含全部核心组件和技能实现
- 适合进行 SDK 修改、技能开发和深度定制

### 从 ooder-Nexus 过渡到本工程

当您在 ooder-Nexus 中完成基础学习后，可以按照以下步骤过渡到本工程：

1. **熟悉核心概念**：确保您已经理解了 Agent、Skill、Scene 等核心概念
2. **了解部署流程**：掌握基本的 Maven 构建和 Spring Boot 应用启动流程
3. **克隆本工程**：
   ```bash
...
4. **构建项目**：
   ...
   ```
5. **运行测试环境**：启动技能服务和代理服务，熟悉完整的协作流程
6. **开始开发**：基于本工程的完整功能进行 SDK 修改或技能开发

### 本工程的优势

与 ooder-Nexus 相比，本工程具有以下优势：

- **完整的组件体系**：包含所有核心组件和技能实现
- **高级功能支持**：提供更复杂的场景测试和功能扩展能力
- **SDK 深度定制**：支持对 SDK 进行深度修改和定制
- **完整的测试环境**：包含多种测试场景和测试工具
- **企业级应用**：适合构建企业级的 AI 能力分发和自动化协作系统

### 适用场景

本工程特别适合以下场景：

- **SDK 开发和定制**：需要修改或扩展 Agent SDK 功能
- **技能开发**：开发自定义技能并集成到 SuperAgent 生态
- **企业级部署**：构建企业内部的 AI 能力分发系统
- **复杂场景测试**：测试和验证复杂的多 Agent 协作场景
- **技术研究**：深入研究 SuperAgent 的核心技术和架构
...
- **依赖管理优化**：
  - 统一了项目依赖管理，避免交叉依赖
  - 从GitHub仓库迁移到Maven Central，使用net.ooder 2.0版本依赖
  - 移除了本地文件依赖，统一使用中央仓库依赖
  - 优化了依赖解析，提高构建速度和可靠性
