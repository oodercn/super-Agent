...
var CapabilityDiscovery = {
    ...
    renderMethods: function() {
        ...
        DISCOVERY_METHODS.forEach(function(method) {
            html += '<div class="method-item" data-method="' + method.id + '" onclick="selectMethod(\'' + method.id + '\')">' +
                '<div class="method-icon" style="background: ' + (method.color || '#64748b') + '20; color: ' + (method.color || '#64748b') + ';">' +
                '<i class="' + (method.icon || 'ri-plug-line') + '"></i></div>' +
                '<div class="method-info">' +
                '<div class="method-name">' + (method.name || method.id) + '</div>' +
                '<div class="method-desc">' + (method.description || method.desc || '') + '</div></div>' +
    ...
    renderDefaultCategories: function() {
        var container = document.getElementById('businessCategoryFilter');
        if (!container) return;
        CategoryService.getUserFacing().then(function(userFacingCategories) {
            var html = '<span class="filter-group-label">业务领域:</span>';
            html += '<span class="filter-chip" data-bc="" onclick="selectBusinessCategory(\'\')" title="全部业务领域"><i class="ri-apps-line"></i> 全部</span>';
            if (userFacingCategories && Array.isArray(userFacingCategories)) {
                userFacingCategories.forEach(function(cat) {
                    html += '<span class="filter-chip" data-bc="' + (cat.code || cat.id) + '" onclick="selectBusinessCategory(\'' + (cat.code || cat.id) + '\')" title="' + (cat.desc || cat.name) + '">' +
                        '<i class="' + cat.icon + '"></i> ' + cat.name + '</span>';
                });
            }
            container.innerHTML = html;
            ...
            container.innerHTML = '<span class="filter-group-label">业务领域:</span><span class="filter-chip active" data-bc="" onclick="selectBusinessCategory(\'\')"><i class="ri-apps-line"></i> 全部</span>';
        });
    },
    ...
    startScan: function() {
        ...
        switch (currentMethod) {
            ...
            case 'GITEE':
                ...
                break;
            case 'SKILL_CENTER':
                CapabilityDiscovery.discoverFromSkillCenter(method);
                break;
            case 'GIT_REPOSITORY':
                CapabilityDiscovery.discoverFromGitRepository(method);
                break;
            case 'AUTO':
                CapabilityDiscovery.discoverAuto(method);
                break;
            case 'LOCAL_FS':
            default:
    ...
    discoverFromSkillCenter: function(method) {
        ...
        console.log('[discoverFromSkillCenter] Starting discovery with config:', config);
        ApiClient.post('/api/v1/discovery/skill-center', config, { timeout: 120000 })
            .then(function(result) {
                console.log('[discoverFromSkillCenter] API response:', result);
                if (result.status === 'success' && result.data) {
                    try {
                        CapabilityDiscovery.processDiscoveryResult(result.data);
                    } catch (e) {
                        console.error('[discoverFromSkillCenter] processDiscoveryResult error:', e);
                        CapabilityDiscovery.addLog('error', '处理结果失败: ' + e.message);
                        CapabilityDiscovery.showEmptyResult();
                    }
                } else {
                    console.error('[discoverFromSkillCenter] Scan failed:', result.message);
                    CapabilityDiscovery.addLog('error', '扫描失败: ' + (result.message || '未知错误'));
                    CapabilityDiscovery.showEmptyResult();
                }
            })
            .catch(function(error) {
                console.error('[discoverFromSkillCenter] Error:', error);
    ...
    discoverFromGitRepository: function(method) {
        ...
        if (!config.repoUrl) {
            CapabilityDiscovery.addLog('error', '请输入 Git 仓库地址');
            CapabilityDiscovery.finishScan();
            return;
        }
        
        console.log('[discoverFromGitRepository] Starting discovery with config:', config);
        ApiClient.post('/api/v1/discovery/git-repository', {
            repoUrl: config.repoUrl,
            branch: config.branch || 'main',
            token: config.token
        }, { timeout: 120000 })
            .then(function(result) {
                console.log('[discoverFromGitRepository] API response:', result);
                if (result.status === 'success' && result.data) {
                    try {
                        CapabilityDiscovery.processDiscoveryResult(result.data);
                    } catch (e) {
                        console.error('[discoverFromGitRepository] processDiscoveryResult error:', e);
                        CapabilityDiscovery.addLog('error', '处理结果失败: ' + e.message);
                        CapabilityDiscovery.showEmptyResult();
                    }
                } else {
                    console.error('[discoverFromGitRepository] Scan failed:', result.message);
                    CapabilityDiscovery.addLog('error', '扫描失败: ' + (result.message || '未知错误'));
                    CapabilityDiscovery.showEmptyResult();
                }
            })
            .catch(function(error) {
                console.error('[discoverFromGitRepository] Error:', error);
                CapabilityDiscovery.addLog('error', '扫描失败: ' + error.message);
                CapabilityDiscovery.showEmptyResult();
            })
            .finally(function() {
    ...
    discoverAuto: function(method) {
        console.log('[discoverAuto] Starting automatic discovery');
        CapabilityDiscovery.addLog('info', '自动检测所有可用发现源...');
        ApiClient.post('/api/v1/discovery/auto', {}, { timeout: 120000 })
            .then(function(result) {
                console.log('[discoverAuto] API response:', result);
                if (result.status === 'success' && result.data) {
                    try {
                        CapabilityDiscovery.processDiscoveryResult(result.data);
                        CapabilityDiscovery.addLog('success', '自动发现完成，共发现 ' + (result.data.capabilities ? result.data.capabilities.length : 0) + ' 个能力');
                    } catch (e) {
                        console.error('[discoverAuto] processDiscoveryResult error:', e);
                        CapabilityDiscovery.addLog('error', '处理结果失败: ' + e.message);
                        CapabilityDiscovery.showEmptyResult();
                    }
                } else {
                    console.error('[discoverAuto] Scan failed:', result.message);
                    CapabilityDiscovery.addLog('error', '扫描失败: ' + (result.message || '未知错误'));
                    CapabilityDiscovery.showEmptyResult();
                }
            })
            .catch(function(error) {
                console.error('[discoverAuto] Error:', error);
                CapabilityDiscovery.addLog('error', '扫描失败: ' + error.message);
                CapabilityDiscovery.showEmptyResult();
            })
            .finally(function() {
    ...
    saveInstallConfig: function(cap) {
        ...
        var sceneGroupData = {
            ...
            name: cap.name,
            description: cap.description || '',
            sceneType: cap.sceneType || cap.id,
            status: 'active'
        };
        
        ApiClient.post('/api/v1/scene-groups', sceneGroupData)
            .then(function(result) {
                if (result.status === 'success') {
                    var sceneGroup = result.data || result;
                    var sceneGroupId = sceneGroup.id || cap.id;
                    
                    console.log('[saveInstallConfig] Scene group created:', sceneGroupId);
                    CapabilityDiscovery.addLog('success', '场景组创建成功: ' + cap.name);
                    ...
                    console.error('[saveInstallConfig] Failed to create scene group:', result.message);
                    CapabilityDiscovery.addLog('error', '场景组创建失败: ' + (result.message || '未知错误'));
                }
            })
            .catch(function(error) {
                console.error('[saveInstallConfig] Error creating scene group:', error);
                CapabilityDiscovery.addLog('error', '场景组创建异常: ' + error.message);
            });
    },

    addMenuForInstalledCapability: function(cap) {
        var currentRole = localStorage.getItem('currentRole') || 'installer';
        var categoryInfo = CapabilityDiscovery.getCategoryInfo(cap.skillForm, cap.sceneType);
