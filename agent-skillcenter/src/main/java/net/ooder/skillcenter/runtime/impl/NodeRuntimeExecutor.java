...
/**
 * Node.js运行时执行器 - 符合v0.7.0协议规范
 */
@Component
public class NodeRuntimeExecutor implements RuntimeExecutor {

    private static final Logger log = LoggerFactory.getLogger(NodeRuntimeExecutor.class);

    private static final List<String> SUPPORTED_VERSIONS = Arrays.asList(
            "16", "18", "20", "22"
    );
    private static final List<String> SUPPORTED_FRAMEWORKS = Arrays.asList(
            "express", "nestjs", "fastify", "plain"
    );
    ...
    public RuntimeStatus initialize(String runtimeId, RuntimeConfig config) {
        RuntimeStatus status = RuntimeStatus.create(runtimeId, "skill", "nodejs", config.getVersion());
        status.setState(RuntimeStatus.RuntimeState.INITIALIZING);
        statuses.put(runtimeId, status);
        configs.put(runtimeId, config);
        log.info("Initialized Node.js runtime {} with version {}", runtimeId, config.getVersion());
        return status;
    }
    ...
    public RuntimeStatus start(String runtimeId) {
        ...
        try {
            log.info("Starting Node.js runtime {} with framework: {}", runtimeId, config.getFramework());
            ...
            status.setMemoryLimit(128 * 1024 * 1024);
            status.setMemoryUsed(16 * 1024 * 1024);
            status.setCpuUsage(0.02);
            
            log.info("Node.js runtime {} started successfully", runtimeId);
        } catch (Exception e) {
            status.setState(RuntimeStatus.RuntimeState.ERROR);
            status.setErrorMessage(e.getMessage());
            log.error("Failed to start Node.js runtime {}: {}", runtimeId, e.getMessage());
        }
    ...
    public RuntimeStatus stop(String runtimeId) {
        ...
        try {
            log.info("Stopping Node.js runtime {}", runtimeId);
            Thread.sleep(20);
            status.setState(RuntimeStatus.RuntimeState.STOPPED);
            log.info("Node.js runtime {} stopped", runtimeId);
        } catch (Exception e) {
            status.setState(RuntimeStatus.RuntimeState.ERROR);
            status.setErrorMessage(e.getMessage());
        }
    ...
    public RuntimeStatus getStatus(String runtimeId) {
        ...
        if (status != null && status.getState() == RuntimeStatus.RuntimeState.RUNNING) {
            status.setLastHeartbeat(new Date());
            status.setCpuUsage(Math.random() * 0.2);
            status.setMemoryUsed((long) (16 * 1024 * 1024 + Math.random() * 32 * 1024 * 1024));
        }
        return status;
    }
    ...
    public Object execute(String runtimeId, String method, Map<String, Object> params) {
        ...
        log.debug("Executing method {} on Node.js runtime {}", method, runtimeId);
        return params;
    }
    ...
    public void destroy(String runtimeId) {
        ...
        log.info("Destroyed Node.js runtime {}", runtimeId);
    }
}
