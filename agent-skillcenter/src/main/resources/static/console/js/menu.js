...
function initMenu(activeSection) {
    fetch('/skillcenter/console/menu-config.json')
...
// 渲染菜单
function renderMenu(menuItems, activeSection) {
...
// 渲染菜单项
function renderMenuItem(item, activeSection) {
    // 检查是否有子菜单项是活动的
...
// 渲染子菜单项
function renderSubMenuItem(item, activeSection) {
...
// 渲染三级子菜单项
function renderSubSubMenuItem(item, activeSection) {
    const isActive = item.id === activeSection;
...
// 渲染默认菜单（当菜单配置加载失败时）
function renderDefaultMenu(activeSection) {
    const sidebar = document.getElementById('sidebar');
    if (!sidebar) return;
