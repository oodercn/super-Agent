...
async function loadMarketSkills() {
    try {
        ...
        // 处理 ApiResponse<PageResponse<SkillListing>> 格式
        const skills = result.data && result.data.content ? result.data.content : (Array.isArray(result) ? result : []);
...
async function loadPopularSkills() {
    try {
        ...
        const result = await response.json();
        // 处理 ApiResponse 格式
        const skills = result.data ? result.data : (Array.isArray(result) ? result : []);
