...
class Workbench {
    ...
    updateUserInfo() {
        ...
        if (userNameEl && this.userInfo) {
            userNameEl.textContent = this.userInfo.name || this.userInfo.userId || '用户';
        }
        if (userAvatarEl && this.userInfo) {
            userAvatarEl.textContent = (this.userInfo.name || this.userInfo.userId || 'U').charAt(0).toUpperCase();
        }
    }
    ...
    renderTodoItem(todo, sceneGroupId) {
        ...
        const isPending = todo.status === 'pending' || todo.status === 'PENDING';
    ...
    renderQuickActions() {
        ...
        const actions = this.workbenchData?.quickActions || [
            ...
            { id: 'qa4', title: '能力管理', url: './my-capabilities.html', icon: 'ri-apps-line' }
        ];
    ...
    renderRecentActivity() {
        ...
        const activities = this.workbenchData?.recentActivity || [
            { type: 'todo', text: '系统初始化完成', time: '刚刚' }
        ];
        ...
        this.workbenchData = {
            ...
            sceneTodoGroups: [
                ...
                },
                {
                    ...
                    highPriorityCount: 0,
                    todos: [
                        { id: 't3', title: '提交周报', priority: 'MEDIUM', status: 'pending' }
                    ]
                }
            ],
            quickActions: [
            ...
            globalTodos: [
                ...
                { id: 'gt2', title: '提交周报', sceneName: '项目协作', priority: 'MEDIUM' }
            ],
            recentActivity: [
