...
北向协议定义了 ENexus（企业级服务中枢）与 MCP Agent（主控代理）之间的通信规范。
...
**绑定流程**：
1. 用户注册 → 创建 MCP Agent 账户
2. MCP Agent 初始化 → 创建 Nexus 服务实例
3. 用户登录 → MCP Agent 认证 → 下发初始化配置包

### 7.3 MCP Agent → ENexus 上报协议

| 协议 | HTTP 方法 | 路径 | 功能 |
|------|----------|------|------|
| 注册上报 | POST | `/api/north/mcp/register` | MCP Agent 注册到 ENexus |
| 心跳上报 | POST | `/api/north/mcp/heartbeat` | 定期心跳保活 |
| 子网上报 | POST | `/api/north/mcp/subnet/report` | 上报子网信息和节点列表 |
| 能力上报 | POST | `/api/north/mcp/capability/report` | 上报 Nexus 能力和技能状态 |
| 状态上报 | POST | `/api/north/mcp/status/report` | 上报运行状态和健康指标 |

### 7.4 ENexus → MCP Agent 命令协议

| 协议 | HTTP 方法 | 路径 | 功能 |
|------|----------|------|------|
| 命令下发 | POST | `/api/north/command/send` | 向 Nexus 发送实时命令 |
| 配置下发 | POST | `/api/north/config/push` | 下发配置更新 |
| 策略下发 | POST | `/api/north/policy/push` | 下发安全策略 |
| 技能下发 | POST | `/api/north/skill/install` | 下发技能安装指令 |

### 7.5 初始化配置包

用户首次登录时，ENexus 生成并发送初始化配置包到 Nexus 节点：

```
初始化配置包
├── 企业服务端点 (SkillCenter, VFS, MSG, Auth, Audit)
├── Agent 配置 (网络、安全、运行时)
├── 场景配置 (自动加入的场景组)
├── 技能配置 (预装技能列表)
├── 安全密钥包
...
1. Nexus A 提交 P2P 申请（申请原因、使用场景、时长/次数）
2. ENexus 进行安全风险评估
3. 管理员审批（同意/拒绝/设置限制）
4. 批准后生成 P2P 连接凭证
5. 使用过程可监控、可撤销
...
| 命令类型 | 说明 | 默认模式 | 允许 P2P |
|---------|------|---------|---------|
| SCENE_INVITE | 场景邀请 | ENexus 转发 | 否 |
| SCENE_JOIN | 加入场景 | ENexus 转发 | 否 |
| SCENE_SYNC | 场景同步 | ENexus 转发 | 是 |
| TASK_ASSIGN | 任务分配 | ENexus 转发 | 否 |
| CAPABILITY_INVOKE | 能力调用 | ENexus 转发 | 是 |
| DATA_SYNC | 数据同步 | ENexus 转发 | 是 |
| FILE_TRANSFER | 文件传输 | ENexus 转发 | 是 |
| MESSAGE_SEND | 发送消息 | ENexus 转发 | 否 |
...
本设计文档提出了ooderNexus南向协议和北向协议的完整功能设计方案，核心亮点包括：

1. **分层协议架构**：清晰的协议分层，支持灵活扩展
2. **全方位可视化**：从物理到逻辑，从2D到3D的多维度可视化
3. **深度安全设计**：从传输到应用，从认证到审计的全栈安全
4. **高性能处理**：支持大规模设备接入和实时数据处理
5. **企业级管理**：用户绑定模型、初始化配置包、安全密钥体系
6. **安全通信架构**：ENexus 转发为主、P2P 直连审批机制

该方案将ooderAgent的南向协议能力最大化，同时通过北向协议实现企业级管理能力，为构建企业级P2P AI能力分发网络提供坚实基础。

---

**相关文档**：
- [ENexus 企业级服务总体设计文档](./docs/ENEXUS_OVERALL_DESIGN.md)
- [SDK 协作需求规格说明书](./docs/SDK_COLLABORATION_REQUIREMENTS.md)
